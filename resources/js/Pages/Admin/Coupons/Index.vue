...
const coupons = [
  { code: 'WELCOME30', type: 'Percentage', value: '30%', minOrder: '৳500', maxDiscount: '৳300', used: 842, limit: 2000, expires: 'Apr 30, 2026', status: 'Active' },
  { code: 'PHYSICS50', type: 'Percentage', value: '50%', minOrder: '৳1,000', maxDiscount: '৳750', used: 312, limit: 500, expires: 'Mar 31, 2026', status: 'Active' },
  { code: 'FLAT200', type: 'Fixed', value: '৳200', minOrder: '৳800', maxDiscount: '৳200', used: 1204, limit: null, expires: 'May 15, 2026', status: 'Active' },
  { code: 'SUMMER25', type: 'Percentage', value: '25%', minOrder: '৳600', maxDiscount: '৳500', used: 0, limit: 1000, expires: 'Jun 30, 2026', status: 'Scheduled' },
  { code: 'RAMADAN40', type: 'Percentage', value: '40%', minOrder: '৳700', maxDiscount: '৳600', used: 3841, limit: 4000, expires: 'Mar 15, 2026', status: 'Expired' },
  { code: 'TRIAL100', type: 'Fixed', value: '৳100', minOrder: '৳300', maxDiscount: '৳100', used: 9821, limit: null, expires: 'Dec 31, 2026', status: 'Active' },
  { code: 'MATH20', type: 'Percentage', value: '20%', minOrder: '৳400', maxDiscount: '৳400', used: 487, limit: 800, expires: 'Apr 20, 2026', status: 'Active' },
]
</script>
  ...
  <AppLayout>
    ...
    <div style="background:#100d09;position:relative;overflow:hidden;padding:28px 26px 26px;flex-shrink:0;">
      ...
      <div style="position:relative;z-index:1;display:flex;align-items:flex-start;justify-content:space-between;gap:18px;flex-wrap:wrap;">
        <div>
          <div class="hero-ey">Promotions</div>
          <h1 class="hero-h">Coupon <em>Codes</em></h1>
          <p class="hero-p">Create and manage discount coupons, track usage, and set expiry rules for promotions.</p>
        </div>
        <div class="kpis">
          <div class="kpi">
            <div class="kpi-v">48</div>
            <div class="kpi-l">Total</div>
            <div class="kpi-d ku">+6</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">31</div>
            <div class="kpi-l">Active</div>
            <div class="kpi-d ku">+3</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">16.5k</div>
            <div class="kpi-l">Total Used</div>
            <div class="kpi-d ku">+842</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">৳38.4k</div>
            <div class="kpi-l">Discounted</div>
            <div class="kpi-d kd">-৳2.1k</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="pb">
      <div class="card">
        <div class="ch">
          <span class="ct">All <em>Coupons</em></span>
          <div class="tb">
            <div class="ts">
              <svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
              <input placeholder="Search coupon code…" />
            </div>
            <select class="tsel">
            ...
            <table class="dt">
              ...
              <tbody>
                <tr v-for="c in coupons" :key="c.code">
                  <td>
                    <span style="font-family:'DM Mono',monospace;font-weight:700;font-size:.8rem;color:#0e0b07;background:#f0e8d6;padding:2px 8px;border-radius:6px;border:1px solid #e0d8cc;">{{ c.code }}</span>
                  </td>
                  <td><span class="badge" :class="c.type === 'Percentage' ? 'bg-p' : 'bg-b'">{{ c.type }}</span></td>
                  <td><span style="font-family:'Playfair Display',serif;font-weight:700;color:#c9893c;">{{ c.value }}</span></td>
                  <td>{{ c.minOrder }}</td>
                  <td>{{ c.maxDiscount }}</td>
                  <td>
                    <div style="display:flex;align-items:center;gap:7px;">
                      <div v-if="c.limit" class="prog">
                        <div class="pf" :style="{ width: (c.used / c.limit * 100) + '%' }"></div>
                      </div>
                      <span style="font-size:.7rem;color:#8a7f72;white-space:nowrap;">{{ c.used.toLocaleString() }}{{ c.limit ? ' / ' + c.limit.toLocaleString() : '' }}</span>
                    </div>
                  </td>
                  <td style="color:#8a7f72;white-space:nowrap;">{{ c.expires }}</td>
                  <td>
                    <span class="badge"
                      :class="c.status === 'Active' ? 'bg-g' : c.status === 'Scheduled' ? 'bg-b' : 'bg-m'">
                      {{ c.status }}
                    </span>
                  </td>
                  <td>
