...
const pendingPayouts = [
  { id: 'PO-3421', teacher: 'Dr. Rahim Uddin', email: 'rahim@example.com', amount: '৳12,480', period: 'Mar 2026', method: 'bKash', account: '01712-XXXXXX', requested: 'Mar 27, 2026', color: '#2d8a5e' },
  { id: 'PO-3420', teacher: 'Ms. Shahnaz Khanam', email: 'shahnaz@example.com', amount: '৳9,840', period: 'Mar 2026', method: 'Nagad', account: '01811-XXXXXX', requested: 'Mar 26, 2026', color: '#c9893c' },
  { id: 'PO-3419', teacher: 'Mr. Kamal Hasan', email: 'kamal@example.com', amount: '৳6,240', period: 'Mar 2026', method: 'Bank', account: 'DBBL-XXXX-1234', requested: 'Mar 26, 2026', color: '#2c5f9e' },
]

const payoutHistory = [
  { id: 'PO-3418', teacher: 'Dr. Sadia Islam', amount: '৳5,460', period: 'Feb 2026', method: 'bKash', processed: 'Mar 5, 2026', status: 'Paid', color: '#6b4aaf' },
  { id: 'PO-3417', teacher: 'Ms. Roksana Parvin', amount: '৳3,720', period: 'Feb 2026', method: 'Nagad', processed: 'Mar 5, 2026', status: 'Paid', color: '#b84b2f' },
  { id: 'PO-3416', teacher: 'Dr. Rahim Uddin', amount: '৳11,040', period: 'Feb 2026', method: 'bKash', processed: 'Mar 5, 2026', status: 'Paid', color: '#2d8a5e' },
  { id: 'PO-3415', teacher: 'Ms. Shahnaz Khanam', amount: '৳8,520', period: 'Feb 2026', method: 'Nagad', processed: 'Mar 5, 2026', status: 'Paid', color: '#c9893c' },
  { id: 'PO-3414', teacher: 'Mr. Jahangir Kabir', amount: '৳1,380', period: 'Feb 2026', method: 'Bank', processed: 'Mar 6, 2026', status: 'Failed', color: '#8a7f72' },
]
</script>
  ...
  <AppLayout>
    ...
    <div style="background:#100d09;position:relative;overflow:hidden;padding:28px 26px 26px;flex-shrink:0;">
      ...
      <div style="position:relative;z-index:1;display:flex;align-items:flex-start;justify-content:space-between;gap:18px;flex-wrap:wrap;">
        <div>
          <div class="hero-ey">Teacher Earnings</div>
          <h1 class="hero-h">Payout <em>Management</em></h1>
          <p class="hero-p">Approve pending teacher payouts, track disbursement history, and manage payment accounts.</p>
        </div>
        <div class="kpis">
          <div class="kpi">
            <div class="kpi-v">৳28.6k</div>
            <div class="kpi-l">Pending</div>
            <div class="kpi-d kw">3 requests</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">৳94.2k</div>
            <div class="kpi-l">Paid Out</div>
            <div class="kpi-d ku">this month</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">72%</div>
            <div class="kpi-l">Margin</div>
            <div class="kpi-d kw">platform</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">28%</div>
            <div class="kpi-l">Teacher Cut</div>
    ...
    <div class="pb">
      ...
      <div class="card">
        <div class="ch">
          <span class="ct">Pending <em>Approvals</em></span>
          <span class="badge bg-o">3 Awaiting</span>
        </div>
        <div class="cb" style="padding:0;">
          <div class="tw">
            <table class="dt">
              ...
              <tbody>
                <tr v-for="p in pendingPayouts" :key="p.id">
                  <td style="font-family:'DM Mono',monospace;font-size:.72rem;color:#8a7f72;">{{ p.id }}</td>
                  <td>
                    <div class="uc">
                      <div class="uav" :style="{ background: p.color }">{{ p.teacher.charAt(0) }}</div>
                      <div>
                        <div class="unm">{{ p.teacher }}</div>
                  ...
                  <td>{{ p.period }}</td>
                  <td><span class="badge bg-b">{{ p.method }}</span></td>
                  <td style="font-family:'DM Mono',monospace;font-size:.7rem;color:#8a7f72;">{{ p.account }}</td>
                  <td style="color:#8a7f72;">{{ p.requested }}</td>
                  <td>
                    <div class="ar">
                      <button class="btn bok bsm">
                        <svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
                        Approve
                      </button>
                      <button class="btn bd bsm">Reject</button>
                    </div>
                  </td>
                </tr>
              </tbody>
      ...
      <div class="card">
        <div class="ch">
          <span class="ct">Payout <em>History</em></span>
          <div class="tb">
            <div class="ts">
            ...
            <table class="dt">
              ...
              <tbody>
                <tr v-for="p in payoutHistory" :key="p.id">
                  ...
                  <td><span style="font-family:'Playfair Display',serif;font-weight:700;">{{ p.amount }}</span></td>
                  <td>{{ p.period }}</td>
                  <td><span class="badge bg-b">{{ p.method }}</span></td>
                  <td style="color:#8a7f72;">{{ p.processed }}</td>
                  <td>
                    <span class="badge" :class="p.status === 'Paid' ? 'bg-g' : 'bg-r'">{{ p.status }}</span>
                  </td>
                  <td>
                    <div class="ar">
                      <button class="btn bg2 bxs">View</button>
                      <button v-if="p.status === 'Failed'" class="btn bok bxs">Retry</button>
                    </div>
                  </td>
                </tr>
              </tbody>
