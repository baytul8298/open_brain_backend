...
const payments = [
  { id: 'PAY-88421', student: 'Arif Hossain', email: 'arif@example.com', course: 'HSC Physics Full Course', amount: '৳1,500', method: 'bKash', gateway: 'TXN-bK-28214', date: 'Mar 26, 2026', status: 'Paid', color: '#2c5f9e' },
  { id: 'PAY-88420', student: 'Tania Begum', email: 'tania@example.com', course: 'SSC Math Master Class', amount: '৳1,200', method: 'Nagad', gateway: 'TXN-NG-19834', date: 'Mar 25, 2026', status: 'Paid', color: '#c9893c' },
  { id: 'PAY-88419', student: 'Rahim Miah', email: 'rahimmiah@example.com', course: 'English Grammar Pro', amount: '৳900', method: 'Card', gateway: 'TXN-CD-77201', date: 'Mar 24, 2026', status: 'Paid', color: '#2d8a5e' },
  { id: 'PAY-88418', student: 'Sana Islam', email: 'sana@example.com', course: 'Chemistry Crash Course', amount: '৳1,100', method: 'bKash', gateway: 'TXN-bK-28100', date: 'Mar 24, 2026', status: 'Pending', color: '#6b4aaf' },
  { id: 'PAY-88417', student: 'Karim Uddin', email: 'karim@example.com', course: 'Biology for HSC', amount: '৳1,000', method: 'Nagad', gateway: 'TXN-NG-19700', date: 'Mar 23, 2026', status: 'Refunded', color: '#b83232' },
  { id: 'PAY-88416', student: 'Nadia Akter', email: 'nadia@example.com', course: 'HSC Physics Full Course', amount: '৳1,500', method: 'bKash', gateway: 'TXN-bK-27980', date: 'Mar 22, 2026', status: 'Paid', color: '#c47a0a' },
  { id: 'PAY-88415', student: 'Fahim Chowdhury', email: 'fahim@example.com', course: 'ICT Basics', amount: '৳600', method: 'Card', gateway: 'TXN-CD-77100', date: 'Mar 21, 2026', status: 'Failed', color: '#8a7f72' },
  { id: 'PAY-88414', student: 'Sumaiya Khan', email: 'sumaiya@example.com', course: 'SSC Math Master Class', amount: '৳1,200', method: 'Nagad', gateway: 'TXN-NG-19601', date: 'Mar 20, 2026', status: 'Paid', color: '#2d8a5e' },
]
</script>
  ...
  <AppLayout>
    ...
    <div style="background:#100d09;position:relative;overflow:hidden;padding:28px 26px 26px;flex-shrink:0;">
      ...
      <div style="position:relative;z-index:1;display:flex;align-items:flex-start;justify-content:space-between;gap:18px;flex-wrap:wrap;">
        <div>
          <div class="hero-ey">Financial Records</div>
          <h1 class="hero-h">Payment <em>Transactions</em></h1>
          <p class="hero-p">Full audit trail of all student payments, gateway references, and refund statuses.</p>
        </div>
        <div class="kpis">
          <div class="kpi">
            <div class="kpi-v">৳142k</div>
            <div class="kpi-l">This Month</div>
            <div class="kpi-d ku">+18%</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">4,821</div>
            <div class="kpi-l">Transactions</div>
            <div class="kpi-d ku">+284</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">৳3.8k</div>
            <div class="kpi-l">Refunded</div>
            <div class="kpi-d kd">+12</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">98.2%</div>
            <div class="kpi-l">Success Rate</div>
            <div class="kpi-d ku">+0.4%</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="pb">
      <div class="card">
        <div class="ch">
          <span class="ct">All <em>Payments</em></span>
          <div class="tb">
            <div class="ts">
              <svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
              <input placeholder="Search by student, ID or gateway ref…" />
            </div>
            <select class="tsel">
            ...
            <table class="dt">
              ...
              <tbody>
                <tr v-for="p in payments" :key="p.id">
                  <td style="font-family:'DM Mono',monospace;font-size:.72rem;color:#8a7f72;">{{ p.id }}</td>
                  <td>
                    <div class="uc">
                      <div class="uav" :style="{ background: p.color }">{{ p.student.charAt(0) }}</div>
                      <div>
                        <div class="unm">{{ p.student }}</div>
                        <div class="uem">{{ p.email }}</div>
                      </div>
                    </div>
                  </td>
                  <td style="max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ p.course }}</td>
                  <td><span style="font-family:'Playfair Display',serif;font-weight:700;color:#0e0b07;">{{ p.amount }}</span></td>
                  <td>
                    <span class="badge bg-b">{{ p.method }}</span>
                  </td>
                  <td style="font-family:'DM Mono',monospace;font-size:.7rem;color:#8a7f72;">{{ p.gateway }}</td>
                  <td style="color:#8a7f72;white-space:nowrap;">{{ p.date }}</td>
                  <td>
                    <span class="badge"
                      :class="p.status === 'Paid' ? 'bg-g' : p.status === 'Pending' ? 'bg-o' : p.status === 'Refunded' ? 'bg-b' : 'bg-r'">
                      {{ p.status }}
                    </span>
                  </td>
                  <td>
                    <div class="ar">
                      ...
                      <button class="btn bd bxs">Refund</button>
                    </div>
                  </td>
                </tr>
              </tbody>
