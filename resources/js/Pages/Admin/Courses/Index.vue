...
const pendingCourses = [
  { name: 'Advanced Trigonometry', teacher: 'Dr. Rahim', subject: 'Mathematics', submitted: '3 hrs ago', lessons: 24 },
  { name: 'Organic Chemistry Vol.2', teacher: 'Dr. Sadia', subject: 'Chemistry', submitted: '8 hrs ago', lessons: 18 },
  { name: 'HSC Biology Genetics', teacher: 'Ms. Parvin', subject: 'Biology', submitted: '1 day ago', lessons: 12 },
]

const allCourses = [
  { name: 'HSC Physics Full Course', teacher: 'Dr. Rahim', subject: 'Physics', price: '৳1,500', students: 1240, lessons: 48, status: 'Published' },
  { name: 'SSC Math Master Class', teacher: 'Ms. Khanam', subject: 'Mathematics', price: '৳1,200', students: 980, lessons: 36, status: 'Published' },
  { name: 'English Grammar Pro', teacher: 'Mr. Hasan', subject: 'English', price: '৳900', students: 760, lessons: 28, status: 'Published' },
  { name: 'Chemistry Crash Course', teacher: 'Dr. Sadia', subject: 'Chemistry', price: '৳1,100', students: 640, lessons: 22, status: 'Published' },
  { name: 'Biology for HSC', teacher: 'Ms. Parvin', subject: 'Biology', price: '৳1,000', students: 520, lessons: 30, status: 'Published' },
  { name: 'Advanced Trigonometry', teacher: 'Dr. Rahim', subject: 'Mathematics', price: '৳800', students: 0, lessons: 24, status: 'Draft' },
  { name: 'ICT Basics', teacher: 'Mr. Kabir', subject: 'ICT', price: '৳600', students: 310, lessons: 16, status: 'Paused' },
  { name: 'Organic Chemistry Vol.2', teacher: 'Dr. Sadia', subject: 'Chemistry', price: '৳1,100', students: 0, lessons: 18, status: 'Draft' },
]
</script>
  ...
  <AppLayout>
    ...
    <div style="background:#100d09;position:relative;overflow:hidden;padding:28px 26px 26px;flex-shrink:0;">
      ...
      <div style="position:relative;z-index:1;display:flex;align-items:flex-start;justify-content:space-between;gap:18px;flex-wrap:wrap;">
        <div>
          <div class="hero-ey">Content Management</div>
          <h1 class="hero-h">Course <em>Catalogue</em></h1>
          <p class="hero-p">Review, publish, and manage all courses across subjects and teachers.</p>
        </div>
        <div class="kpis">
          <div class="kpi">
            <div class="kpi-v">618</div>
            <div class="kpi-l">Total</div>
            <div class="kpi-d ku">+24</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">541</div>
          ...
          <div class="kpi">
            <div class="kpi-v">74</div>
            <div class="kpi-l">Drafts</div>
    ...
    <div class="pb">
      ...
      <div class="card">
        <div class="ch">
          <span class="ct">Pending <em>Review</em></span>
        ...
        <div class="cb" style="padding:0;">
          <div class="tw">
            <table class="dt">
              ...
              <tbody>
                <tr v-for="c in pendingCourses" :key="c.name">
                  <td><span style="font-weight:600;">{{ c.name }}</span></td>
                  <td>{{ c.teacher }}</td>
                  <td><span class="badge bg-b">{{ c.subject }}</span></td>
                  <td>{{ c.lessons }} lessons</td>
                  <td style="color:#8a7f72;">{{ c.submitted }}</td>
                  <td>
                    <div class="ar">
                      <button class="btn bbl bsm">Preview</button>
                      <button class="btn bok bsm">Publish</button>
      ...
      <div class="card">
        <div class="ch">
          <span class="ct">All <em>Courses</em></span>
          <div class="tb">
            <div class="ts">
        ...
        <div class="cb" style="padding:0;">
          <div class="tw">
            <table class="dt">
              ...
              <tbody>
                <tr v-for="c in allCourses" :key="c.name + c.teacher">
                  <td><span style="font-weight:600;">{{ c.name }}</span></td>
                  <td>{{ c.teacher }}</td>
                  <td><span class="badge bg-b">{{ c.subject }}</span></td>
                  <td><span style="font-family:'Playfair Display',serif;">{{ c.price }}</span></td>
                  <td>{{ c.students.toLocaleString() }}</td>
                  <td>{{ c.lessons }}</td>
                  <td>
                    <span class="badge"
                      :class="c.status === 'Published' ? 'bg-g' : c.status === 'Draft' ? 'bg-m' : 'bg-o'">
                      {{ c.status }}
                    </span>
                  </td>
                  <td>
                    <div class="ar">
                      ...
                      <button class="btn bd bxs">Unpublish</button>
                    </div>
                  </td>
                </tr>
              </tbody>
