...
const liveSessions = [
  { id: 'LSS-2201', title: 'Electromagnetic Induction', teacher: 'Dr. Rahim', course: 'HSC Physics Full Course', started: '14 mins ago', viewers: 312, status: 'Live' },
  { id: 'LSS-2200', title: 'Calculus: Integration by Parts', teacher: 'Ms. Khanam', course: 'SSC Math Master Class', started: '38 mins ago', viewers: 198, status: 'Live' },
  { id: 'LSS-2199', title: 'Organic Chemistry: Polymers', teacher: 'Dr. Sadia', course: 'Chemistry Crash Course', started: '1 hr ago', viewers: 87, status: 'Live' },
]

const upcoming = [
  { id: 'LSS-2202', title: 'Thermodynamics Q&A', teacher: 'Dr. Rahim', course: 'HSC Physics Full Course', scheduled: 'Mar 28, 2026 — 5:00 PM', registered: 284 },
  { id: 'LSS-2203', title: 'Trigonometry Mock Exam', teacher: 'Ms. Khanam', course: 'SSC Math Master Class', scheduled: 'Mar 29, 2026 — 4:00 PM', registered: 210 },
  { id: 'LSS-2204', title: 'DNA Replication Deep Dive', teacher: 'Ms. Parvin', course: 'Biology for HSC', scheduled: 'Mar 30, 2026 — 6:00 PM', registered: 156 },
  { id: 'LSS-2205', title: 'Essay Writing Masterclass', teacher: 'Mr. Hasan', course: 'English Grammar Pro', scheduled: 'Apr 1, 2026 — 3:30 PM', registered: 128 },
  { id: 'LSS-2206', title: 'Database Design Intro', teacher: 'Mr. Kabir', course: 'ICT Basics', scheduled: 'Apr 2, 2026 — 5:30 PM', registered: 72 },
]
</script>
  ...
  <AppLayout>
    ...
    <div style="background:#100d09;position:relative;overflow:hidden;padding:28px 26px 26px;flex-shrink:0;">
      ...
      <div style="position:relative;z-index:1;display:flex;align-items:flex-start;justify-content:space-between;gap:18px;flex-wrap:wrap;">
        <div>
          <div class="hero-ey">Real-Time Classroom</div>
          <h1 class="hero-h">Live <em>Sessions</em></h1>
          <p class="hero-p">Monitor active livestreams, manage upcoming sessions, and track viewer engagement.</p>
        </div>
        <div class="kpis">
          <div class="kpi">
            <div class="kpi-v">3</div>
            <div class="kpi-l">Live Now</div>
            <div class="kpi-d kw">ongoing</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">597</div>
            <div class="kpi-l">Viewers</div>
            <div class="kpi-d ku">+42</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">5</div>
            <div class="kpi-l">Upcoming</div>
            <div class="kpi-d kw">scheduled</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">8.2k</div>
    ...
    <div class="pb">
      ...
      <div class="card">
        <div class="ch">
          <span class="ct">Currently <em>Live</em></span>
          <span class="badge bg-r" style="animation:none;">3 Active Streams</span>
        </div>
            ...
            <table class="dt">
              ...
              <tbody>
                <tr v-for="s in liveSessions" :key="s.id">
                  <td><span style="font-weight:600;">{{ s.title }}</span><br><span style="font-family:'DM Mono',monospace;font-size:.68rem;color:#8a7f72;">{{ s.id }}</span></td>
                  <td>{{ s.teacher }}</td>
                  <td style="max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ s.course }}</td>
                  <td style="color:#8a7f72;">{{ s.started }}</td>
                  <td>
                    <span style="font-family:'Playfair Display',serif;font-size:.88rem;color:#0e0b07;">{{ s.viewers }}</span>
                  </td>
                  <td><span class="badge bg-r">● Live</span></td>
                  <td>
                    <div class="ar">
                      <button class="btn bbl bsm">Watch</button>
                      <button class="btn bd bsm">End</button>
                    </div>
                  </td>
                </tr>
              </tbody>
      ...
      <div class="card">
        <div class="ch">
          <span class="ct">Upcoming <em>Sessions</em></span>
          <div class="tb">
            <div class="ts">
            ...
            <table class="dt">
              ...
              <tbody>
                <tr v-for="s in upcoming" :key="s.id">
                  <td style="font-family:'DM Mono',monospace;font-size:.72rem;color:#8a7f72;">{{ s.id }}</td>
                  <td><span style="font-weight:600;">{{ s.title }}</span></td>
                  <td>{{ s.teacher }}</td>
                  <td style="max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ s.course }}</td>
                  <td style="color:#8a7f72;white-space:nowrap;">{{ s.scheduled }}</td>
                  <td>{{ s.registered }}</td>
                  <td>
                    <div class="ar">
                      <button class="btn bg2 bxs">Edit</button>
                      <button class="btn bd bxs">Cancel</button>
                    </div>
                  </td>
                </tr>
              </tbody>
