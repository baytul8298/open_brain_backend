...
const reviews = [
  { id: 'REV-4821', student: 'Arif Hossain', course: 'HSC Physics Full Course', rating: 5, comment: 'Absolutely brilliant course! Dr. Rahim explains every concept with crystal clarity. Highly recommended for HSC students.', date: 'Mar 26, 2026', status: 'Published', color: '#2c5f9e' },
  { id: 'REV-4820', student: 'Tania Begum', course: 'SSC Math Master Class', rating: 4, comment: 'Very structured content. The practice problems are excellent. Would love more video explanations for harder topics.', date: 'Mar 25, 2026', status: 'Published', color: '#c9893c' },
  { id: 'REV-4819', student: 'Karim Uddin', course: 'ICT Basics', rating: 2, comment: 'Videos are too short and content is outdated. Expected more hands-on projects and examples.', date: 'Mar 24, 2026', status: 'Flagged', color: '#b83232' },
  { id: 'REV-4818', student: 'Nadia Akter', course: 'Chemistry Crash Course', rating: 5, comment: 'Dr. Sadia is amazing. The crash course covered everything I needed for my exams. Got an A+!', date: 'Mar 23, 2026', status: 'Published', color: '#c47a0a' },
  { id: 'REV-4817', student: 'Rahim Miah', course: 'English Grammar Pro', rating: 1, comment: 'Spam review content removed.', date: 'Mar 22, 2026', status: 'Hidden', color: '#8a7f72' },
  { id: 'REV-4816', student: 'Sumaiya Khan', course: 'Biology for HSC', rating: 4, comment: 'Great diagrams and clear explanations. The section on genetics was especially well done.', date: 'Mar 21, 2026', status: 'Published', color: '#2d8a5e' },
  { id: 'REV-4815', student: 'Fahim Chowdhury', course: 'HSC Physics Full Course', rating: 5, comment: "Best physics course available in Bangladesh. Dr. Rahim's problem-solving approach is unmatched.", date: 'Mar 20, 2026', status: 'Published', color: '#6b4aaf' },
]
</script>
  ...
  <AppLayout>
    ...
    <div style="background:#100d09;position:relative;overflow:hidden;padding:28px 26px 26px;flex-shrink:0;">
      ...
      <div style="position:relative;z-index:1;display:flex;align-items:flex-start;justify-content:space-between;gap:18px;flex-wrap:wrap;">
        <div>
          <div class="hero-ey">Student Feedback</div>
          <h1 class="hero-h">Course <em>Reviews</em></h1>
          <p class="hero-p">Moderate student reviews, manage flagged content, and maintain quality standards.</p>
        </div>
        <div class="kpis">
          <div class="kpi">
            <div class="kpi-v">18.4k</div>
            <div class="kpi-l">Total</div>
            <div class="kpi-d ku">+284</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">4.8★</div>
            <div class="kpi-l">Avg Rating</div>
            <div class="kpi-d kw">stable</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">42</div>
            <div class="kpi-l">Flagged</div>
            <div class="kpi-d kd">+8</div>
          </div>
          <div class="kpi">
            <div class="kpi-v">128</div>
            <div class="kpi-l">Hidden</div>
            <div class="kpi-d kw">—</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="pb">
      <div class="card">
        <div class="ch">
          <span class="ct">All <em>Reviews</em></span>
          <div class="tb">
            <div class="ts">
            ...
            <table class="dt">
              ...
              <tbody>
                <tr v-for="r in reviews" :key="r.id">
                  <td>
                    <div class="uc">
                      <div class="uav" :style="{ background: r.color }">{{ r.student.charAt(0) }}</div>
                      <div class="unm">{{ r.student }}</div>
                    </div>
                  </td>
                  <td style="max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ r.course }}</td>
                  <td>
                    <span style="color:#c9893c;font-weight:700;font-family:'Playfair Display',serif;">{{ '★'.repeat(r.rating) }}<span style="color:#e8ddc8;">{{ '★'.repeat(5 - r.rating) }}</span></span>
                  </td>
                  <td style="max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#8a7f72;font-size:.74rem;">{{ r.comment }}</td>
                  <td style="color:#8a7f72;white-space:nowrap;">{{ r.date }}</td>
                  <td>
                    <span class="badge"
                      :class="r.status === 'Published' ? 'bg-g' : r.status === 'Flagged' ? 'bg-r' : 'bg-m'">
                      {{ r.status }}
                    </span>
                  </td>
                  <td>
                    <div class="ar">
                      <button class="btn bok bxs">Approve</button>
                      <button class="btn bd bxs">Hide</button>
                    </div>
                  </td>
                </tr>
              </tbody>
