...
import GradeLevelModal from '@/Components/GradeLevel/GradeLevelModal.vue'
...
const gradeLevelToDelete = ref<GradeLevel | null>(null)
...
function applyFilters() {
  ...
  router.get('/grade-level', params, { preserveState: true, preserveScroll: true })
}
...
function askDelete(g: GradeLevel) {
  gradeLevelToDelete.value = g
  showDeleteConfirm.value = true
}

function doDelete() {
  if (!gradeLevelToDelete.value) return
  router.delete(`/grade-level/${gradeLevelToDelete.value.id}`, {
    preserveScroll: true,
    onSuccess: () => {
      showDeleteConfirm.value = false
      gradeLevelToDelete.value = null
    },
  })
}
...
<template>
  ...
  <AppLayout>
    ...
    <div style="background:#100d09;position:relative;overflow:hidden;padding:28px 26px 26px;flex-shrink:0;">
      ...
      <div style="position:relative;z-index:1;display:flex;align-items:flex-start;justify-content:space-between;gap:18px;flex-wrap:wrap;">
        <div>
          ...
          <h1 class="hero-h">Grade <em>Levels</em></h1>
          <p class="hero-p">Manage grade levels shown on the student registration page and across the platform.</p>
        </div>
        <div class="kpis">
          <div class="kpi">
    ...
    <div class="pb">
      <div class="card">
        <div class="ch">
          <span class="ct">All <em>Grade Levels</em></span>
          <div class="tb">
            <div class="ts">
              <svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
              <input v-model="searchQuery" placeholder="Search grade levels…" />
            </div>
        ...
        <div class="cb" style="padding:0;">
          <div v-if="gradeLevels.data.length === 0" style="padding:40px;text-align:center;color:#8a7f72;font-size:.82rem;">
          ...
          <div v-else class="tw">
            <table class="dt">
              ...
              <tbody>
                <tr v-for="g in gradeLevels.data" :key="g.id">
                  <td>
                    <div style="display:flex;align-items:center;gap:9px;">
                      <div :style="{ width:'28px', height:'28px', borderRadius:'7px', background: g.color ?? '#c9893c', flexShrink:0 }"></div>
                      <div>
                        <div style="font-weight:600;">{{ g.name }}</div>
                        <div v-if="g.name_bn" style="font-size:.68rem;color:#8a7f72;">{{ g.name_bn }}</div>
                      </div>
                    </div>
                  </td>
                  <td style="font-family:'DM Mono',monospace;font-size:.72rem;color:#8a7f72;">{{ g.slug ?? '—' }}</td>
                  <td>
                    <div style="display:flex;align-items:center;gap:6px;">
                      <div :style="{ width:'14px', height:'14px', borderRadius:'4px', background: g.color ?? '#ccc', border:'1px solid rgba(0,0,0,.1)' }"></div>
                      <span style="font-family:'DM Mono',monospace;font-size:.72rem;color:#8a7f72;">{{ g.color ?? '—' }}</span>
                    </div>
                  </td>
                  <td style="font-size:.78rem;color:#8a7f72;">{{ g.icon ?? '—' }}</td>
                  <td style="font-size:.78rem;color:#8a7f72;">{{ g.sort_order }}</td>
                  <td>
                    <span class="badge" :class="g.is_active ? 'bg-g' : 'bg-m'">
                      {{ g.is_active ? 'Active' : 'Inactive' }}
                    </span>
                  </td>
                  <td>
                    <div class="ar">
                      <button class="btn bg2 bxs" @click="openEdit(g)">Edit</button>
                      <button class="btn bd bxs" @click="askDelete(g)">Delete</button>
                    </div>
                  </td>
                </tr>
              </tbody>
        ...
        <!-- Pagination -->
        <div v-if="gradeLevels.last_page > 1" style="display:flex;align-items:center;justify-content:space-between;padding:11px 17px;border-top:1px solid #e0d8cc;font-size:.75rem;color:#8a7f72;">
          <span>Showing {{ gradeLevels.from }}–{{ gradeLevels.to }} of {{ gradeLevels.total }}</span>
          <div style="display:flex;gap:4px;">
            <button v-for="p in gradeLevels.last_page" :key="p"
              @click="goPage(p)"
              class="btn bxs"
              :class="p === gradeLevels.current_page ? 'bp' : 'bg2'"
      ...
      <Transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="showDeleteConfirm" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="showDeleteConfirm = false">
          <div style="background:#faf6ef;border:1.5px solid #e0d8cc;border-radius:17px;width:min(480px,93vw);padding:24px;" @click.stop>
            <h3 style="font-family:'Playfair Display',serif;font-size:1.08rem;color:#0e0b07;margin-bottom:8px;">Delete Grade Level</h3>
