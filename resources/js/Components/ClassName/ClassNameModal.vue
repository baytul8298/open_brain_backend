...
watch(() => props.className, (c) => {
  if (c && props.mode === 'edit') {
    form.name       = c.name
    form.slug       = c.slug
    form.name_bn    = c.name_bn ?? ''
    form.icon       = c.icon ?? ''
    form.color      = c.color ?? '#c9893c'
...
const title = computed(() => props.mode === 'create' ? 'Add Class Name' : 'Edit Class Name')
...
function handleSubmit() {
  ...
  if (props.mode === 'create') {
    form.transform(() => payload).post('/class-name', {
      preserveScroll: true,
      onSuccess: () => { emit('success'); emit('close') },
    })
  } else if (props.className) {
    form.transform(() => payload).put(`/class-name/${props.className.id}`, {
      preserveScroll: true,
      onSuccess: () => { emit('success'); emit('close') },
    })
  }
          ...
          <div v-if="open" class="mc" @click.stop>
            ...
            <form @submit.prevent="handleSubmit" class="mf">
              ...
              <div class="fg">
                ...
                <input class="fi" v-model="form.slug" placeholder="e.g. class-6" :disabled="form.processing" required />
              ...
              <div class="fg" style="flex-direction:row;align-items:center;gap:10px;margin-top:2px;">
                ...
                <span style="font-size:.75rem;color:#8a7f72;">Inactive class names are hidden from students</span>
              </div>
              ...
              <div class="mb2">
                ...
                <button type="submit" class="btn bp" :disabled="form.processing">
                  {{ form.processing ? 'Saving…' : (mode === 'create' ? 'Add Class Name' : 'Save Changes') }}
                </button>
              </div>
