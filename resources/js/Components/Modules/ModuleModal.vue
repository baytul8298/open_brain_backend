...
const form = useForm({ module_name: '' })

watch(() => props.module, (m) => {
  if (m && props.mode === 'edit') form.module_name = m.module_name
  else form.reset()
}, { immediate: true })
...
const modalTitle = computed(() => props.mode === 'create' ? 'Create Module' : 'Edit Module')

function handleSubmit() {
  if (props.mode === 'create') {
    form.post('/modules', { preserveScroll: true, onSuccess: () => { emit('success'); emit('close') } })
  } else if (props.module) {
    form.put(`/modules/${props.module.id}`, { preserveScroll: true, onSuccess: () => { emit('success'); emit('close') } })
  }
}
          ...
          <div v-if="open" class="mc" @click.stop>
            ...
            <form @submit.prevent="handleSubmit" class="mf">
              <div class="fg" style="margin-bottom:16px">
                <label class="fl">Module Name *</label>
                <input class="fi" v-model="form.module_name" placeholder="e.g., User Management" :disabled="form.processing" required/>
                <span v-if="form.errors.module_name" class="fe">{{ form.errors.module_name }}</span>
                <span class="fhint">Enter a unique name for this module</span>
              </div>
              <div class="mb2">
                ...
                <button type="submit" class="btn bp" :disabled="form.processing">
                  {{ form.processing ? 'Saving…' : (mode === 'create' ? 'Create Module' : 'Save Changes') }}
                </button>
              </div>
            </form>
          </div>
