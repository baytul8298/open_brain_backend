...
function handleEdit(m: Module) { emit('edit', m) }
function handleDelete(m: Module) { emit('delete', m) }
...
function goToPage(page: number) {
  if (page < 1 || page > props.modules.last_page) return
  router.visit(`/modules?page=${page}`, { preserveState: true, preserveScroll: true })
}
  ...
  <div class="card">
    ...
    <div class="ch">
      <div class="tb" style="flex:1">
        <div class="ts">
          ...
          <input v-model="search" placeholder="Module name…"/>
        </div>
      </div>
    </div>
    ...
    <div class="tw">
      <table class="dt">
        ...
        <tbody>
          <tr v-for="mod in modules.data" :key="mod.id">
            <td>
              <div class="mnm">{{ mod.module_name }}</div>
            </td>
            <td>
              <span class="pcnt">{{ mod.permissions_count ?? '—' }}</span>
            </td>
            <td style="color:#8a7f72;font-size:.74rem">{{ formatDate(mod.created_at) }}</td>
            <td style="color:#8a7f72;font-size:.74rem">{{ formatDate(mod.updated_at) }}</td>
            <td>
              <div class="ar">
                <button class="btn bg2 bxs" @click="handleEdit(mod)">Edit</button>
                <button class="btn bd bxs" @click="handleDelete(mod)">Delete</button>
              </div>
            </td>
          </tr>
          <tr v-if="!modules.data.length">
            <td colspan="5" style="text-align:center;padding:24px;color:#8a7f72;font-size:.8rem;">No modules found</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="cb" style="padding-top:9px;padding-bottom:9px">
      <div class="pagi">
        <span class="pinfo">{{ (modules.current_page - 1) * modules.per_page + 1 }}–{{ Math.min(modules.current_page * modules.per_page, modules.total) }} of {{ modules.total }}</span>
        <button class="pb2" @click="goToPage(modules.current_page - 1)">‹</button>
        <button
          v-for="page in pages(modules.current_page, modules.last_page)"
          :key="page"
          :class="['pb2', page === modules.current_page ? 'on' : '']"
          @click="goToPage(page)"
        >{{ page }}</button>
        <button class="pb2" @click="goToPage(modules.current_page + 1)">›</button>
      </div>
