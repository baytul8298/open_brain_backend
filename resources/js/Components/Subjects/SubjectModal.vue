...
interface Props {
  ...
  allSubjects: { id: number; name: string; color: string | null }[]
}
...
watch(() => props.subject, (s) => {
  if (s && props.mode === 'edit') {
    ...
    form.color      = s.color ?? '#c9893c'
    form.icon       = s.icon ?? ''
    form.parent_id  = s.parent_id
...
watch(() => form.name, (val) => {
  if (props.mode === 'create') {
    form.slug = val.toLowerCase().trim().replace(/[^a-z0-9\u0980-\u09FF]+/g, '-').replace(/^-|-$/g, '')
  }
})

const title = computed(() => props.mode === 'create' ? 'Add Subject' : 'Edit Subject')

// Filtered parent options — exclude current subject from its own parent list
const parentOptions = computed(() =>
  props.allSubjects.filter(s => props.mode !== 'edit' || s.id !== props.subject?.id)
)
...
function handleSubmit() {
  ...
  if (props.mode === 'create') {
    form.transform(() => payload).post('/subjects', {
      preserveScroll: true,
      onSuccess: () => { emit('success'); emit('close') },
    })
  } else if (props.subject) {
    form.transform(() => payload).put(`/subjects/${props.subject.id}`, {
      preserveScroll: true,
      onSuccess: () => { emit('success'); emit('close') },
    })
  }
            ...
            <form @submit.prevent="handleSubmit" class="mf">
              ...
              <div class="frow">
                <div class="fg">
                  ...
                  <input class="fi" v-model="form.name" placeholder="e.g. Mathematics" :disabled="form.processing" required />
                  <span v-if="form.errors.name" class="fe">{{ form.errors.name }}</span>
                </div>
                <div class="fg">
                  <label class="fl">Bangla Name</label>
                  <input class="fi" v-model="form.name_bn" placeholder="e.g. গণিত" :disabled="form.processing" />
              ...
              <div class="fg">
                ...
                <input class="fi" v-model="form.slug" placeholder="e.g. mathematics" :disabled="form.processing" required />
                <span v-if="form.errors.slug" class="fe">{{ form.errors.slug }}</span>
                <span class="fhint">URL-friendly identifier, auto-filled from name</span>
              </div>
              ...
              <div class="frow">
                ...
                <div class="fg">
                  ...
                  <input class="fi" v-model="form.icon" placeholder="e.g. calculator" :disabled="form.processing" />
              ...
              <div class="frow">
                <div class="fg">
                  <label class="fl">Parent Subject</label>
                  <select class="fi fs" v-model="form.parent_id" :disabled="form.processing">
                    <option :value="null">— None —</option>
                    <option v-for="s in parentOptions" :key="s.id" :value="s.id">{{ s.name }}</option>
                  </select>
                  <span v-if="form.errors.parent_id" class="fe">{{ form.errors.parent_id }}</span>
                </div>
                <div class="fg">
              ...
              <div class="fg" style="flex-direction:row;align-items:center;gap:10px;margin-top:2px;">
                ...
                <span style="font-size:.75rem;color:#8a7f72;">Inactive subjects are hidden from students</span>
              </div>
              ...
              <div class="mb2">
                ...
                <button type="submit" class="btn bp" :disabled="form.processing">
                  {{ form.processing ? 'Saving…' : (mode === 'create' ? 'Add Subject' : 'Save Changes') }}
                </button>
              </div>
